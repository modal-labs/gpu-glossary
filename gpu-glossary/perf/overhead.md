...
Unlike time spent [bottlenecked](/gpu-glossary/perf/FIXME) on [compute](/gpu-glossary/perf/FIXME) or [memory](/gpu-glossary/perf/FIXME), during which the GPU is working as fast as possible, latency from overhead represents time where the GPU is instead waiting to receive work.
