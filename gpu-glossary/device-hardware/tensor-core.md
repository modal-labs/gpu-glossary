...
The `16816` between is not a single number larger than 16,000. Instead, the string of
numbers `16`, `8`, and `16` denote the dimensions of the matrices. These dimensions
are generally named `m`, `n`, and `k` by NVIDIA, for example in
[PTX](/gpu-glossary/device-software/parallel-thread-execution) instructions. The
outer dimensions of A and B, aka `m` and `n`, come first,
followed by the shared inner dimension for the accumulation, `k`.
